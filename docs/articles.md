## Полезные ссылки по Golang
### Базовые вещи
-	[Документация по Golang](https://golang.org/cmd/go/)	
-	[Спецификация по языку](https://golang.org/ref/spec)
-	[Вики Golang Github](https://github.com/golang/go/wiki)
-	[golang-book](http://golang-book.ru/)   
-	[Постулаты Go. Маленькая статья об основными принципах языка](https://habr.com/ru/post/272383/)
### Статьи по структурам, особенностям и нюансам их использования
-	[Базовые типы данных в Golang](https://research.swtch.com/godata)
- [Структурные типы данных в Golang](http://golang-book.ru/chapter-06-arrays-slices-maps.html) 
-	[Строки, байты, руны и символы в Golang](https://blog.golang.org/strings)
-	[Массивы, слайсы и строки](https://blog.golang.org/slices)
-	[Слайсы использование и устройство](https://blog.golang.org/go-slices-usage-and-internals)
-	[Функции в Golang](http://jordanorelli.com/post/42369331748/function-types-in-go-golang)
-	[Map Golang в примерах](https://blog.golang.org/go-maps-in-action)
-	[Интерфейсы в Golang](https://research.swtch.com/interfaces)
-	[Краш-курс по интерфейсам в Go](https://habrahabr.ru/post/276981/)  
-	[Интерфейсы в GO простым языком](https://nuancesprog.ru/p/14201/)
### Codestyle, Bestpracties
-	[Эффективный Go](https://golang.org/doc/effective_go.html)
-	[50 оттенков golang](https://habr.com/ru/company/mailru/blog/314804/)
-	[Материал аналогичный 50 оттенков golang](https://divan.dev/posts/avoid_gotchas/)
-	[Как ревьювить (и писать код). обязательно к прочтению](https://github.com/golang/go/wiki/CodeReviewComments)
-	[Строки: лучшие практики](https://itnext.io/mastering-strings-in-golang-best-practices-for-efficient-coding-72306021d796)
-	[Лучшие практики go](https://habr.com/ru/company/mailru/blog/301036/)
-	[Как организовать код(Видео)](https://www.youtube.com/watch?v=MzTcsI6tn-0)
-	[Организация кода в go](https://habrahabr.ru/post/308198/)
-	[Как не наступать на грабли в Go](https://habrahabr.ru/post/325468/)
-	[Советы по огранизации Golang кода](https://blog.golang.org/organizing-go-code)
- [Хитрости и проблемы Golang](https://habr.com/ru/company/mailru/blog/314804/)  
### Пакеты и их организация
-	[Разбираемся в Go: пакет io](https://habr.com/ru/post/306914/)
-	[Пакеты и зависимости](https://medium.com/@benbjohnson/standard-package-layout-7cdbc8391fc1)
- [Про организацию кода. GOPATH и пакеты](https://golang.org/doc/code.html)
### Память
- [Модель памяти golang](https://golang.org/ref/mem)
-	[Утечки памяти часть 1](https://habr.com/ru/company/ncloudtech/blog/675390/)
-	[Утечки памяти часть 2](https://habr.com/ru/company/ncloudtech/blog/676960/)
-	[Зачем в Go амперсанд и звёздочка (& и *)](https://habr.com/ru/post/339192/)
- [Сборщик мусора(видео-доклад)](www.youtube.com/watch?v=CX4GSErFenI)
### Многопоточность
-	[Многопоточность и каналы](https://habr.com/ru/post/141853/)
- [Под капотом Golang — как работают каналы. Часть 1](https://medium.com/@victor_nerd/%D0%BF%D0%BE%D0%B4-%D0%BA%D0%B0%D0%BF%D0%BE%D1%82%D0%BE%D0%BC-golang-%D0%BA%D0%B0%D0%BA-%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%B0%D1%8E%D1%82-%D0%BA%D0%B0%D0%BD%D0%B0%D0%BB%D1%8B-%D1%87%D0%B0%D1%81%D1%82%D1%8C-1-e1da9e3e104d)
- [Под капотом Golang — как работают каналы. Часть 2](https://medium.com/@victor_nerd/golang-channel-internal-part2-b4e37ad9a118)
### Тестирование
-	[Стуктурирование тестов](https://medium.com/@benbjohnson/structuring-tests-in-go-46ddee7a25c)
### HOWTO и примеры
-	[Работа с файлами](https://www.devdungeon.com/content/working-files-go)
- [Как настроить автодеплой бинарей в github](https://web3.coach/golang-github-actions-release-binaries-tutorial) 
-	[Быстрый кеш на Go Map](https://blog.stackademic.com/writing-a-partitioned-cache-using-go-map-x3-faster-than-the-standard-map-dbfe704fe4bf)
-	[Много how-to касательно базовых вещей в go](https://www.golangprograms.com/)
-	[Набор how-to где можно получить углублённую информацию по всем базовым вещам. очень полезно](https://yourbasic.org/golang/)
-	[Большая подборка статей](https://github.com/enocom/gopher-reading-list)
-	[Пример создания web-сервера на golang](https://eli.thegreenplace.net/2021/rest-servers-in-go-part-1-standard-library/)
-	[Работа с контекстом в GO](https://habr.com/ru/company/nixys/blog/461723/)
-	[Контекст в Go](https://dev.to/karanpratapsingh/understand-context-in-go-46em)
-	[Сборник готовых примеров](http://yourbasic.org/golang/blueprint/)
-	[Примеры структур данных на Golang](https://github.com/Workiva/go-datastructures)

## Литература по golang на русском языке:  
-	Язык программирования Go, Алан А. А. Донован, Брайан У. Керниган
-	Go на практике, Matt Butcher, Мэтт Фарина Мэтт
-	Программирование на Go. Разработка приложений XXI века, Марк Саммерфильд  

## Инструменты для подготовки к собеседованиям
-	[Тест с вопросами для собеседования на golang](https://proghub.ru/t/golang-interview)  
- [Презентация с возможностью выполнять код онлайн](talks.godoc.org/github.com/davecheney/presentations/gopher-puzzlers.slide#1)  
- [Площадка с возможностью создавать опросники, с логгированием действий отвечающего](https://www.interviewzen.com/)
